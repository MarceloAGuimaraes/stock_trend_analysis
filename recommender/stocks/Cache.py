...
class Cache:
    ...
    def __init__(self, cache_folder="/home/marcelo/Projects/stock_trend_analysis/data"):
    ...
    def load_stock_data(
        self, symbols, stocks=None, ticker=None, cache=True, load_data=True
    ):
        ...
        for symbol in symbols:
            ...
            if symbol in stocks and load_data:
                ...
                try:
                    df_stock = ticker.historic(
                        symbol, start=None, resolution="daily"
                    ).reset_index()
                    # store it as filw
                    df_stock.to_csv(
                        os.path.join(self.path, "Stocks", "{}.txt".format(symbol))
                    )
                except:
    ...
    def load_statement_data(
        ...
        if statement is None and load_missing == True:
            warnings.warn(
                "No statement instance given, no additional data can be loaded! Set `load_missing` to False or pass an instance to silence this warning."
            )
