...
def income(symbol, period="annual"):
    ...
    if period == "quarter":
        meta["period"] = period
    url = utils.build_url("income-statement", symbol, meta=meta)
...
def growth(symbol, period="annual"):
    ...
    if period == "quarter":
        meta["period"] = period
    url = utils.build_url("financial-growth", symbol, meta=meta)
    data = utils.fetch(url)
    ...
    if df.empty:
        ...
        return utils.convert_dtype(
            df, "string", non_cols=["date", "symbol", "period", "calendarYear"]
        )
